...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (o.status === OrderStatus.REFUNDED) return { already: true, order: o };
      if (o.status === OrderStatus.CANCELLED) throw new Error("CANNOT_REFUND");
    ...
    if (m === "CANNOT_REFUND") return NextResponse.json({ error: "CANNOT_REFUND" }, { status: 409 });
    console.error("MARK_REFUNDED_ERROR", e);
